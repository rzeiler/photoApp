...
myApp.controller('photoController', ['$scope', '$sce', '$http', '$routeParams', '$location', '$timeout', function($scope, $sce, $http, $routeParams, $location, $timeout) {
	...
	$scope.edit = function edit(arg) {
		$location.path("/photo/" + $routeParams.id + "/edit/" + arg);
	};
	$scope.delete = function(arg, src) {
		if(confirm("Delete, You are sure?")) {
			$http.post("tph.php", {
          ...
          if(json.removed_picture == 1) {
  					var card = document.getElementById(json.id);
  					card.remove();
  				}
				}
				if(json.error) {
